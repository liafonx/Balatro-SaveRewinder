...
## [1.5.0] - 2026-02-07
### 新增
- **收藏存档列表（书签）**：现在可以标记重要存档，并按收藏存档筛选列表。
- **批量标记流程**：标记模式支持先预览改动，再一次性应用，待提交标记改动显示 `[?]`。
- **收藏存档图标**：在存档列表为 **编辑收藏存档** 功能添加星形图标按钮

### 更改
- **存档面板操作区**：存档列表底部改为 **打开收藏列表**、**★ 编辑收藏存档图标**、**▶ 当前存档图标** 三个按钮（列表内 **全部删除** 已移除）。
- **按钮视觉更新**：**跳转到当前存档**按钮改为三角形图标按钮。
- **存档保留策略**：关键存档在超过上限清理中会被保留。

### 改进
- **大量存档性能**：在存档数量较多时，创建存档和打开列表更流畅。
...
### 更改
- **Mod压缩包结构** — 更改了 zip 文件结构：模组文件现在包装在 `SaveRewinder` 文件夹中，而不是位于压缩包根目录

> ℹ️ **注意**：此版本**不包含新功能或错误修复**。仅更改了发布 zip 结构以便于安装。如果你在安装或更新到 1.4.6 版本时遇到任何问题，请使用此版本。否则，如果你已经成功运行 1.4.6，**无需更新**。

> 📦 **Thunderstore 用户**：Thunderstore 上传的压缩包结构保持不变。
